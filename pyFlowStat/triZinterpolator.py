"""
Interpolation inside triangular grids without z as a member of the classes.
"""
...
class LinearTriZInterpolator(TriZInterpolator):
    """
    ...
    def __init__(self, triangulation, z, trifinder=None):
        TriZInterpolator.__init__(self, triangulation, trifinder)

        # Store plane coefficients for fast interpolation calculations.
        self._plane_coefficients = \
            self._triangulation.calculate_plane_coefficients(z)

    def __call__(self, x, y):
        return self._interpolate_multikeys(x, y, tri_index=None,
                                           return_keys=('z',))[0]
    __call__.__doc__ = TriZInterpolator.docstring__call__

    def gradient(self, x, y):
        return self._interpolate_multikeys(x, y, tri_index=None,
                                           return_keys=('dzdx', 'dzdy'))
    gradient.__doc__ = TriZInterpolator.docstringgradient
...
class CubicTriZInterpolator(TriZInterpolator):
    """
    ...
    :class:`~matplotlib.tri.CubicTriZInterpolator` computes a cubic
    interpolation.
    ...
    a simple geometric approximation is used (weighted average of the triangle
    normal vectors), which could improve speed on very large grids.
...
#                               compressed_y/float(self._unit_y))).T
        ...
        self._dof = self._compute_dof(z, dz=dz)
        # Loading HCT element
    ...
    def _compute_dof(self, z, dz=None):
        """
        ...
        """        
        TE = _DOF_estimator_geom(self,z)
        return TE.compute_dof_from_df(z)
...
class _DOF_estimator():
    ...
    """
    def __init__(self, interpolator, z, **kwargs):
        if not isinstance(interpolator, CubicTriZInterpolator):
...
#        self.z = interpolator._z
        ...
        self.dz = self.compute_dz(z,**kwargs)
        self.compute_dof_from_df(z)

    def compute_dz(self, **kwargs):
        raise NotImplementedError

    def compute_dof_from_df(self,z):
        """
        Computes reduced-HCT elements degrees of freedom, knowing the
        gradient.
        """
        J = CubicTriZInterpolator._get_jacobian(self._tris_pts)
        tri_z = z[self._triangles]
...
class _DOF_estimator_geom(_DOF_estimator):
    ...
    def compute_dz(self,z):
        ...
        el_geom_grad = self.compute_geom_grads(z)
